...
void MyEventAction::EndOfEventAction(const G4Event *event) {
  ...
  if (hcID < 0)
    return; // If the collection doesn't exist, do nothing.

  auto hitsCollection = static_cast<GratingHitsCollection *>(
      event->GetHCofThisEvent()->GetHC(hcID));
  if (!hitsCollection || hitsCollection->entries() == 0)
    return; // If no hits, do nothing.
  ...
  for (size_t i = 0; i < hitsCollection->entries(); ++i) {
    ...
    if (hit->GetDetectorNb() == 1)
      hitG1_passed = true;
    if (hit->GetDetectorNb() == 2)
      hitG2_passed = true;
    if (hit->GetDetectorNb() == 3)
      hitC3 = true;
  ...
  // Bin 4: C3 Hit
  if (hitG1_absorbed)
    manager->FillH1(gratingHistId, 0);
  if (hitG1_passed)
    manager->FillH1(gratingHistId, 1);

  if (hitG2_absorbed)
    manager->FillH1(gratingHistId, 2);
  if (hitG2_passed)
    manager->FillH1(gratingHistId, 3);

  if (hitC3)
    manager->FillH1(gratingHistId, 4);
  ...
  if (fHadAntiprotonSiliconCollision) {
    ...
    // Fill count and type histograms for each secondary particle
    manager->FillH1(manager->GetH1Id("SecondaryParticleCount"),
    ...
    for (const auto &particleList : fSecondaryParticlesMap) {
      for (const auto &particle : particleList.second) {
        ...
        if (particle.particleName == "gamma")
          ...
          typeCode = 2;
        else if (particle.particleName == "pi0")
          typeCode = 3;
        else if (particle.particleName == "proton")
          typeCode = 4;
...
void MyEventAction::StoreIntraModuleMomentum(const G4Track *track, G4int copyNo,
                                             const G4ThreeVector &momentum) {
  fIntraModuleMomentumMap[track][copyNo] = momentum;
}

G4ThreeVector MyEventAction::GetIntraModuleMomentum(const G4Track *track,
                                                    G4int copyNo) const {
...
void MyEventAction::ClearIntraModuleMomentum(const G4Track *track,
                                             G4int copyNo) {
  auto trackIt = fIntraModuleMomentumMap.find(track);
  if (trackIt != fIntraModuleMomentumMap.end()) {
...
void MyEventAction::StoreInterModuleMomentum(const G4Track *track,
                                             const G4ThreeVector &momentum) {
...
G4ThreeVector
MyEventAction::GetInterModuleMomentum(const G4Track *track) const {
...
void MyEventAction::StoreSingleScintMomentum(G4int trackID,
                                             const G4ThreeVector &momentum) {
...
void MyEventAction::StoreB2BFrontMomentum(G4int trackID,
                                          const G4ThreeVector &momentum) {
