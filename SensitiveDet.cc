...
void MySensitiveDetector::Initialize(G4HCofThisEvent *hce) {
  ...
  fGratingHitsCollection =
      new GratingHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool MySensitiveDetector::ProcessHits(G4Step *aStep,
  ...
  // This logic is prioritized. If a hit is in a grating, we process it here and
  // stop.
  // =======================================================================
  // We only care about the first moment a primary particle (TrackID==1) ENTERS
  // a volume.
  if (preStepPoint->GetStepStatus() == fGeomBoundary &&
    ...
    if (volumeName == "GratingOpeningLog") {
      // Particle passed through a grating opening. Get the mother's copy number
      // (1 or 2).
  ...
  if (volumeName == "SolidCounterLog") {
    ...
    if (track->GetParentID() == 0 &&
      ...
      // 1. We fill the "MoireProfile" (X-Projection) because the pattern varies
      // along X.
  ...
  if (volumeName == "ScintillatorLV" || volumeName == "ScintillatorSingleLV") {
    ...
    if (particleName == "pi+" || particleName == "pi-" ||
        particleName == "pi0" || particleName == "anti_proton") {
