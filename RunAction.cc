...
void MyRunAction::BeginOfRunAction(const G4Run *) {
  ...
  manager->CreateH3("SourceXYZDistribution", "Primary Particle Source Position",
                    50, -12.0, -4.0, 50, 0.0, 7.0, 101, -50.5, 50.5);

  CreateHistogramWithTitles(manager, "ScintillatorHits",
                            "Scintillator Copy Numbers", 400, 0, 400);
  CreateHistogramWithTitles(manager, "PionEnergyDep",
                            "Energy Deposition by Pions", 100, 0, 10);

  manager->CreateH2("Edep2DByProcess", "Energy Deposition by Process", 13, 0,
                    13, 100, 0, 10);
  manager->CreateH1("IntraModuleDeviation", "Intra-module angular deviation",
                    40, 0., 2.0);
  manager->CreateH1("InterModuleDeviation", "Inter-module angular deviation",
                    40, 0., 2.0);
  manager->CreateH1("SingleScintDeviation",
                    "Angular Deviation in Test Scintillator", 40, 0, 2.0);
  manager->CreateH1("TwoScintB2BDeviation",
                    "Angular Deviation Between B2B Scintillators", 50, 0., 25.);
  ...
  manager->CreateH1("PrimaryCount", "Number of Primary Particles per Event", 10,
                    0, 10);

  // FIX 2 & 3: ADD HIGH-RESOLUTION Y-PROJECTION
  // This is the histogram your analysis script "analyze_contrast.C" is looking
  // for! 7000 bins = 10 micrometer resolution over 7 cm.
  manager->CreateH1("MoirePatternY",
                    "Moiré Pattern (Y-Projection);Y (cm);Counts", 7000, -3.5,
                    3.5);

  // You can keep these for visual checks, but they are not for contrast
  // analysis
  manager->CreateH2("MoirePattern", "Moiré Pattern (2D)", 200, -3.5, 3.5, 50,
                    -3.5, 3.5);
  // Increase bins from 200 to 7000 to resolve 100um fringes!
  manager->CreateH1("MoireProfile", "Moiré Profile (X-Projection)", 7000, -3.5,
                    3.5);
...
std::ofstream &MyRunAction::GetPionFile() { return fPionFile; }

std::ofstream &MyRunAction::GetPionFile() const {
  // const_cast is safe here because fPionFile is mutable in practice (file
  // stream)
  return const_cast<std::ofstream &>(fPionFile);
}

void MyRunAction::CreateHistogramWithTitles(G4AnalysisManager *manager,
                                            const G4String &name,
                                            const G4String &title, G4int bins,
