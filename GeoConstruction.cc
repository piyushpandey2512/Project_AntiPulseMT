...
MyDetectorConstruction::MyDetectorConstruction() { DefineCommands(); }
...
G4VPhysicalVolume *MyDetectorConstruction::Construct() {
  ...
  G4VPhysicalVolume *physWorld = new G4PVPlacement(
      0, G4ThreeVector(), wLogic, "World", 0, false, 0, overlapCheck);
  ...
  if (useSTLGeometry) {
    ...
    G4String stlFile = "/home/piyush/Desktop/PhD_Work/Trento_Project/"
                       "Project_AntiPulse/stl_geometry/TotalGravModuleV2.stl";
    ...
    G4LogicalVolume *stlLogical =
        new G4LogicalVolume(stlSolid, mat304steel, "STLVolume");
    ...
    new G4PVPlacement(rotation, stlPosition, stlLogical, "STLVolume", wLogic,
  ...
  if (useMoireGratingSetup) {
    cout << "GeoConstruction: Using 2 HORIZONTAL Moiré gratings + 1 Solid "
            "Counter setup"
         << endl;
    ...
    G4Box *gratingMother_box = new G4Box("GratingMotherBox", grating_halfX,
                                         grating_halfY, grating_halfZ);
    G4LogicalVolume *gratingMother_log =
        new G4LogicalVolume(gratingMother_box, Gal_mat, "GratingMotherLog");

    // Build one slice of the grating (one pitch) and replicate it
    G4Box *slice_box =
        new G4Box("SliceBox", grating_halfX, pitch / 2.0, grating_halfZ);
    G4LogicalVolume *slice_log =
        new G4LogicalVolume(slice_box, Gal_mat, "SliceLog");

    // Create the replicas along Y
    G4int num_replicas = G4int((2.0 * grating_halfY) / pitch);
    new G4PVReplica("GratingReplica", slice_log, gratingMother_log, kYAxis,
                    num_replicas, pitch);

    // Now build the wall and opening within the slice
    G4Box *wall_box =
        new G4Box("WallBox", grating_halfX, wall_width / 2.0, grating_halfZ);
    fGratingWallLogical = new G4LogicalVolume(wall_box, fSiMaterial, "WallLog");
    G4Box *opening_box = new G4Box("OpeningBox", grating_halfX,
                                   opening_width / 2.0, grating_halfZ);
    fGratingOpeningLogical =
        new G4LogicalVolume(opening_box, Gal_mat, "GratingOpeningLog");

    // Place the wall and opening within the slice
    // G4double wall_pos_y = -opening_width / 2.0;
    // G4double opening_pos_y = wall_width / 2.0;
    G4double opening_pos_y =
        -pitch / 2.0 + opening_width / 2.0;               // Results in -30 um
    G4double wall_pos_y = pitch / 2.0 - wall_width / 2.0; // Results in +20 um
    new G4PVPlacement(0, G4ThreeVector(0, wall_pos_y, 0), fGratingWallLogical,
                      "Wall", slice_log, false, 0, true);
    new G4PVPlacement(0, G4ThreeVector(0, opening_pos_y, 0),
                      fGratingOpeningLogical, "Opening", slice_log, false, 0,
                      true);
    fGratingWallLogical->SetVisAttributes(
        new G4VisAttributes(G4Colour::Magenta()));
    fGratingOpeningLogical->SetVisAttributes(
    ...
    G4Box *solidCounter_box = new G4Box("SolidCounterBox", grating_halfX,
                                        grating_halfY, counter_halfZ);
    fSolidCounterLogical =
        new G4LogicalVolume(solidCounter_box, fSiMaterial, "SolidCounterLog");

    // --- MODIFICATION: Create new, SOLID visualization attributes for the
    // counter ---
    ...
    G4ThreeVector center_counter =
        stlPosition + G4ThreeVector(0, 0, 45 * cm + counter_halfZ);
    ...
    new G4PVPlacement(rot, center_grating1, gratingMother_log, "MoireGrating",
                      wLogic, false, 1, true);

    // Place the second grating (at Z=-45) with copy number 2
    new G4PVPlacement(0, center_grating2, gratingMother_log, "MoireGrating",
                      wLogic, false, 2, true);

    // Place the third grating (at Z=+45) with copy number 3
    // new G4PVPlacement(0, center_grating3, gratingMother_log, "MoireGrating",
    // wLogic, false, 3, true);

    // Place the SOLID COUNTER (at Z=+45) with copy number 3
    new G4PVPlacement(0, center_counter, fSolidCounterLogical, "SolidCounter",
                      wLogic, false, 3, true);
  }
  ...
  if (useFourModuleSetupNewFEE) {
    ...
    // G4double scinHalfX = 2.5*cm; /// test for energy deposition with double
    // width G4double scinHalfX = 1.25*cm / 2.0; // test for energy deposition
    // with half width
    G4double scinHalfY = 0.6 * cm / 2.0;
    G4double scinHalfZ = 50.0 * cm / 2.0;

    G4Box *scinBox =
        new G4Box("ScintillatorBox", scinHalfX, scinHalfY, scinHalfZ);
    fScintLogical =
        new G4LogicalVolume(scinBox, fScinMaterial, "ScintillatorLV");
    ...
    for (size_t m = 0; m < modulePositions.size(); m++) {
      ...
      for (G4int j = 0; j < 13; j++) {
        ...
        new G4PVPlacement(0, scintPos, fScintLogical, "Scintillator", wLogic,
                          false, m * 100 + j, overlapCheck);
      }
    }
  }
  ...
  if (useTestScintillator) {
    ...
    G4Box *oneScinBox =
        new G4Box("ScintillatorBox", scinHalfX, scinHalfY, scinHalfZ);
    oneScinLogical =
        new G4LogicalVolume(oneScinBox, fScinMaterial, "ScintillatorSingleLV");
    oneScinLogical->SetVisAttributes(visAttScin);

    G4ThreeVector oneScinPos1(10 * cm, 0, 0);
    // G4ThreeVector oneScinPos2(20*cm, 0, 0);

    new G4PVPlacement(0, oneScinPos1, oneScinLogical, "OneScintillator1",
                      wLogic, false, 0, overlapCheck);
    // new G4PVPlacement(0, oneScinPos2, oneScinLogical, "OneScintillator2",
    // wLogic, false, 0, overlapCheck);
  }
  ...
  if (useTestModulesSetup) {
    ...
    G4double scinHalfX = 2.5 * cm / 2.0;
    // // G4double scinHalfX = 2.5*cm; /// test for energy deposition with
    // double width G4double scinHalfX = 1.25*cm / 2.0; // test for energy
    // deposition with half width
    G4double scinHalfY = 0.6 * cm / 2.0;
    G4double scinHalfZ = 50.0 * cm / 2.0;
  ...
  if (useTwoScinB2B) {
    ...
    // width
    G4double scinHalfX =
        1.25 * cm / 2.0; // test for energy deposition with half width
    G4double scinHalfY = 0.6 * cm / 2.0;
    G4double scinHalfZ = 50.0 * cm / 2.0;
    ...
    new G4PVPlacement(0, oneScinPos1, fScintLogical, "OneScintillator1", wLogic,
                      false, 0, overlapCheck);
    new G4PVPlacement(0, oneScinPos2, fScintLogical, "OneScintillator2", wLogic,
                      false, 0, overlapCheck);
  }
...
void MyDetectorConstruction::ConstructSDandField() {
  ...
  // Assign this single instance to all logical volumes that should be
  // sensitive.
  if (fScintLogical != nullptr)
    SetSensitiveDetector(fScintLogical, masterSD);
  if (oneScinLogical != nullptr)
    SetSensitiveDetector(oneScinLogical, masterSD);
  if (fGratingWallLogical != nullptr)
    SetSensitiveDetector(fGratingWallLogical, masterSD);
  if (fGratingOpeningLogical != nullptr)
    SetSensitiveDetector(fGratingOpeningLogical, masterSD);
  if (fSolidCounterLogical != nullptr)
    SetSensitiveDetector(fSolidCounterLogical, masterSD);
  // =======================================================================

  // // 1. Define the gravity field
  // G4UniformGravityField* gravityField = new
  // G4UniformGravityField(G4ThreeVector(0, -9.81*m/s/s, 0));
