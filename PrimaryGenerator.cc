...
// Add these as class members in PrimaryGenerator.hh or as static/global for
// quick testing
...
MyPrimaryParticles::~MyPrimaryParticles() { delete fParticleGun; }

void MyPrimaryParticles::GeneratePrimaries(G4Event *anEvent) {
  if (anEvent->GetEventID() % 1000 == 0) {
    G4cout << "PrimaryGenerator: Generating event " << anEvent->GetEventID()
           << G4endl;
  }
  ...
  if (useAntiprotonBeamCone) {
    if (!antiproton) {
      G4Exception("PrimaryGenerator::GeneratePrimaries", "MyCode0001",
                  FatalException, "Particle 'anti_proton' not found.");
      return;
    }
  ...
  if (useLightBeamParallel) {
    ...
    G4ParticleDefinition *photon = particleTable->FindParticle("gamma");
    if (!photon) {
      G4Exception("PrimaryGenerator::GeneratePrimaries", "MyCode0002",
                  FatalException, "Particle 'gamma' not found.");
      return;
    }

    // --- 2. Define Beam Properties ---
    G4double beam_energy = 3.0 * eV; // Visible light ~413 nm
    G4double beam_radius =
        5.0 * cm; // A 5cm radius circle fully covers a 7cm x 7cm square
    G4double start_z =
        -60.0 * cm; // Start 10cm before the first grating at z=-50cm
  ...
  if (useAntiprotonBeamRandomAiming) {
    ...
    G4double target_radius =
        7.0 * cm;                   // A radius big enough to cover the gratings
    G4double target_z = -50.0 * cm; // The Z position of your first grating

    // --- 3. Generate a Random Position on the SOURCE Disk ---
    // This logic is excellent and remains unchanged. It correctly gives a
    // uniform distribution over the area of the circle.
    ...
    // The direction is simply the vector pointing from the start position to
    // the aim point. We normalize it with .unit() to make it a direction vector
    // of length 1.
    ...
    fParticleGun->SetParticleMomentumDirection(
        final_direction); // Use the new random direction
  ...
  if (useThreeSourceCone) {
    ...
    G4ThreeVector stlPosition(
        -8.0 * cm, 3.5 * cm,
        0.0 * cm); // Adjusted for better alignment with modules
    ...
    std::vector<G4ThreeVector> sourcePositions = {
        stlPosition, stlPosition + G4ThreeVector(0, 0, 45.0 * cm),
    ...
    std::vector<G4ThreeVector> moduleCenters = {
        G4ThreeVector(10 * cm, 0, 30 * cm), G4ThreeVector(20 * cm, 0, 30 * cm),
    ...
    for (const auto &sourceCenter : sourcePositions) {
      ...
      // Select particle type based on annihilation ratios (same as
      // useMoireSource)
      ...
      if (rand < 0.60) { // Charged pions (60% of total)
        ...
        energy =
            150 * MeV + 250 * MeV * G4UniformRand(); // Uniform distribution
      ...
      G4ThreeVector randomDirection(sinTheta * std::cos(phi),
                                    sinTheta * std::sin(phi), randomCosTheta);
  ...
  if (useMoireSourceDiagnostic) {
    ...
    // 5. Run your reconstruction code and record the "Total extrapolated
    // vertices found".
    // 6. Come back here, comment out the first test case, and uncomment the
    // second one.
    // 7. Repeat steps 3, 4, and 5.
    // 8. Compare the number of vertices from both runs. If the code and
    // geometry
    ...
    // const G4ThreeVector testAimPoint(25.8 * cm, 0.0, 30.0 * cm); // Aims at
    // right arm

    // --- Test Case 2: Negative Z Source ONLY ---
    const G4ThreeVector testSourceCenter(-8.0 * cm, 3.5 * cm, -50.0 * cm);
    const G4ThreeVector testAimPoint(25.8 * cm, 0.0,
                                     -30.0 * cm); // Aims at left arm
    // =================================================================

    // --- Common settings for the test ---
    std::vector<G4ThreeVector> sourcePositions = {
        testSourceCenter}; // Use only the selected test source
    ...
    // This loop will only run once per event, for the single source defined
    // above.
    for (const auto &sourceCenter : sourcePositions) {
      ...
      // Define the cone axis from the particle's origin to the pre-defined aim
      // point
      ...
      // Rotate this randomly generated vector so that it is oriented along the
      // coneAxis
      G4ThreeVector finalDirection = randomDirection.rotateUz(coneAxis);
  ...
  if (useMoireSourceRandomSource) {
    ...
    // Define the average centers of the LEFT and RIGHT detector arms for
    // aiming.
    ...
    // This ensures a balanced generation from all three sources over the entire
    // run.
    G4int sourceID =
        static_cast<G4int>(G4UniformRand() * sourcePositions.size());
    ...
    // --- MODIFICATION: Fill ntuple with source vertex position AND the Source
    // ID --- We assume your ntuple ID is 2, and columns are X, Y, Z, SourceID.
    ...
    analysisManager->FillNtupleIColumn(
        2, 3, sourceID); // Store which source it came from (0, 1, or 2)
    analysisManager->AddNtupleRow(2);

    // Fill the 3D histogram with the source position.
    auto h3id = analysisManager->GetH3Id("SourceXYZDistribution");
    analysisManager->FillH3(h3id, sourcePos.x() / cm, sourcePos.y() / cm,
                            sourcePos.z() / cm);
    ...
    if (rand < 0.60) {
      particle = (G4UniformRand() < 0.5) ? particleTable->FindParticle("pi+")
      ...
      particle = (G4UniformRand() < 0.5) ? particleTable->FindParticle("kaon+")
                                         : particleTable->FindParticle("kaon-");
      energy = 150 * MeV + 250 * MeV * G4UniformRand();
    } else {
    ...
    } else if (sourceCenter.z() <
               -4 * cm) { // This is the Z=-50 cm source (sourceID 2)
    ...
    // --- CONE EMISSION LOGIC ---
    // 1. Define the cone axis from the particle's actual origin to the aim
    // point.
    ...
    // 3. Rotate this randomly generated vector so it's oriented along the
    // coneAxis.
    G4ThreeVector finalDirection = randomDirection.rotateUz(coneAxis);
  ...
  if (useMoireSourceUniform) {
    ...
    // Define the dimensions of the source box (using your physically correct
    // thick source)
    G4double boxHalfX = 7.0 * cm / 2.0;
    G4double boxHalfY = 7.0 * cm / 2.0;
    G4double boxHalfZ = 250.0 * micrometer / 2.0;
    ...
    // 2. Define the opening angle of the emission cone. This is a key parameter
    // to tune.
    //    A larger angle ensures all detectors are illuminated, especially from
    ...
    for (const auto &sourceCenter : sourcePositions) {
      ...
      // 3. Define the cone axis: a vector from the particle's actual origin to
      // the detector center.
      //    This must be calculated for each particle because `sourcePos` is
      //    randomized.
      G4ThreeVector coneAxis = (aimPoint - sourcePos).unit();

      // 4. Sample a random direction within the defined cone using the provided
      // logic.
      G4double cosThetaMax = std::cos(coneHalfAngle);
      G4double cosTheta = cosThetaMax + (1.0 - cosThetaMax) * G4UniformRand();
      G4double sinTheta = std::sqrt(1.0 - cosTheta * cosTheta);
      ...
      // 5. Rotate this randomly generated vector so that it is oriented along
      // the coneAxis.
      //    `rotateUz` finds the rotation that maps the Z-axis to the coneAxis
      //    and applies it.
      G4ThreeVector finalDirection = randomDirection.rotateUz(coneAxis);
      ...
      fParticleGun->SetParticlePosition(sourcePos);
      fParticleGun->SetParticleMomentumDirection(
          finalDirection); // Use the new direction
      fParticleGun->SetParticleEnergy(energy);
  ...
  if (useMoireSourceGaussian) {
    ...
    std::vector<G4ThreeVector> localSourcePositions = {
        G4ThreeVector(0, 0, 0), G4ThreeVector(0, 0, 45 * cm),
    ...
    for (const auto &sourceCenter : sourcePositions) {
      ...
      // (The rest of the particle selection, aiming, and generation logic is
      // unchanged)
      // ...
  ...
  if (useMoireSourceGaussianIsotropic) {
    ...
    // (Detector and other definitions are unchanged, but we remove the aiming
    // logic) G4ThreeVector rightArmCenter(25.8 * cm, 0.0,  30.0 * cm); //
    // REMOVED G4ThreeVector leftArmCenter (25.8 * cm, 0.0, -30.0 * cm); //
    // REMOVED G4double coneHalfAngle = 40 * deg; // REMOVED
    ...
    for (const auto &sourceCenter : sourcePositions) {
      ...
      G4double cosTheta =
          2.0 * G4UniformRand() - 1.0; // cos(theta) uniform in [-1, 1]
      G4double sinTheta = std::sqrt(1.0 - cosTheta * cosTheta);
      G4double phi = 2.0 * M_PI * G4UniformRand(); // phi uniform in [0, 2*pi]

      G4ThreeVector finalDirection(sinTheta * std::cos(phi),
                                   sinTheta * std::sin(phi), cosTheta);
      ...
      fParticleGun->SetParticlePosition(sourcePos);
      fParticleGun->SetParticleMomentumDirection(
          finalDirection); // Use the new isotropic direction
      fParticleGun->SetParticleEnergy(energy);
  ...
  if (useTestSingleSource) {
    ...
    fParticleGun->SetParticleMomentumDirection(
        (G4ThreeVector(1, 0, 0)).unit()); // Forward direction
    fParticleGun->GeneratePrimaryVertex(anEvent);
  }
